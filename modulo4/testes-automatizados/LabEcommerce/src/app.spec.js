...
describe("Home Page tests", () => {
  ...
  test("Indicador que inicial existe um produto com nome legal", () => {
    const { getByText } = render(<App />);
    const indicador = getByText(/Produto legal/i); // case insensitive
    // expect(indicador.textContent).toBe("Produto legal");
    // expect(indicador).toHaveTextContent("Quantidade de produtos: 3"); // ou assim
  });

  test("Indicador que inicial existe um valor de R$ 200,00", () => {
    const { getByText } = render(<App />);
    const indicador = getByText(/200/); // case insensitive
    // expect(indicador.textContent).toBe("Produto legal");
    // expect(indicador).toHaveTextContent("Quantidade de produtos: 3"); // ou assim
  });
